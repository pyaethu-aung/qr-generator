...
export function TelForm({ config, onNumberChange }: TelFormProps) {
  ...
  // The QR encodes the normalized digits, not what's typed. Surface that string so the
  // user can confirm the number that will actually be dialed, and nudge for a country
  // code when it's missing (a bare local number may not dial outside its region).
  const dialNumber = useMemo(() => normalizePhone(config.number), [config.number])
  const missingCountryCode = !!dialNumber && !dialNumber.startsWith('+')
      ...
      <div className="flex flex-col gap-1.5">
        ...
        {dialNumber && (
          <p className="text-xs text-text-secondary">
            {translate('controls.telDialPreview').replace('{number}', dialNumber)}
          </p>
        )}
        {missingCountryCode && (
          <p aria-live="polite" className="text-xs text-warning">
            {translate('controls.telNoCountryCodeCaution')}
          </p>
        )}
      </div>
    </div>
