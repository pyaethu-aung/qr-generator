...
export const Textarea = forwardRef<HTMLTextAreaElement, TextareaProps>(
  ({ label, helperText, error, fullWidth = true, className, id, rows = 3, disabled, 'aria-describedby': ariaDescribedBy, ...props }, ref) => {
    ...
    const textareaClass = twMerge(
      clsx(
        'block resize-none rounded-lg border border-border-strong bg-surface-inset px-3 py-2 text-sm text-text-primary shadow-sm placeholder:text-text-disabled',
